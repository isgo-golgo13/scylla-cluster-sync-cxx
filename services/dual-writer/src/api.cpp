...
void start_api_server(uint16_t port,
    ...
    // --- GET /status ---
    svr.Get("/status", [&filter](const httplib::Request&, httplib::Response& res) {
        json body;
        body["service"] = "dual-writer";
        body["status"]  = "running";
        ...
        if (filter) {
            ...
            body["filter"]["tables_skipped"]  = stats.tables_skipped.load(std::memory_order_relaxed);
            body["filter"]["tenants_skipped"] = stats.tenants_skipped.load(std::memory_order_relaxed);
            body["filter"]["rows_allowed"]    = stats.rows_allowed.load(std::memory_order_relaxed);
        }
